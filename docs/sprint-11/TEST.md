# TEST.md — kochfabrik Sprint 11 (TDD-Stubs aus EARS)

> Stubs aus den EARS-Kriterien von FEATURE-004/005 — initial ROT.
> Ablage: `backend/tests/test_sprint11.py` (US-047+, auf dem
> Monorepo-Branch) bzw. `backend/tests/test_charakterisierung.py`
> (US-046, eigener TDD-Zyklus laut Story). Wave-1-Stories mit
> Doc-Artefakten (US-044/045) nutzen ihre Verify-Blöcke als Gate.
...
**US-046 (Charakterisierung)** hat seinen eigenen TDD-Zyklus:
`backend/tests/test_charakterisierung.py` — TestClient DB-los:
Health-Routen-Status/Shape, Auth-Gate (401 ohne Cookie), statisches
Asset. Stubs zuerst rot (Datei existiert nicht), dann grün — und der
Alembic-Namespace-Fix macht die GESAMTE Suite erstmals lokal 0-failed.

**Arbeitsauftrag /sprint-execute:** `test_sprint11.py` wird auf dem
Branch `sprint-11-monorepo` MIT US-047 angelegt (rot vor dem Umbau,
grün nach US-050). Wave-1-Branches fassen ihn nicht an.
